 ...
 * @param io_type 
*/
void CUdpClientSession::start(const CConnect_IO_Info& io_info)
{
    //初始化send/recv缓冲区
    ...
    //建立连接
    udp::endpoint end_point(asio::ip::address::from_string( io_info.server_ip.c_str()), io_info.server_port );
    this->send_endpoint_ = end_point;
    
    this->socket_.open(udp::v4());
    this->socket_.set_option(asio::ip::udp::socket::reuse_address(true));
    
    //判断有没有本地IP
    if (io_info.client_ip.size() > 0 && io_info.client_port > 0)
    {
        udp::endpoint local_endpoint(asio::ip::address::from_string( io_info.client_ip.c_str() ), io_info.client_port ); // 本地地址和端口，0.0.0.0表示任意地址
        recv_endpoint_ = local_endpoint;
        try
        {
            this->socket_.bind(local_endpoint);    // 将套接字绑定到本地地址和端口
        }
        catch (std::system_error const& ex)
        {
            PSS_LOGGER_ERROR("[CUdpClientSession::start] bind addr error remote:[{}:{}] local:[{}:{}] ex.what:{}.", io_info.server_ip , io_info.server_port , io_info.client_ip , io_info.client_port , ex.what() );
        }
    }

    //建立链接
    asio::error_code connect_error;
    this->socket_.connect( end_point , connect_error);
    ...
    else
    {
        ...
        remote_ip.m_strClientIP = this->socket_.remote_endpoint().address().to_string();
        remote_ip.m_u2Port = this->socket_.remote_endpoint().port();
        local_ip.m_strClientIP = this->socket_.local_endpoint().address().to_string();
        local_ip.m_u2Port = this->socket_.local_endpoint().port();
        ...
        this->packet_parse_interface_->packet_connect_ptr_( this->connect_id_ , remote_ip , local_ip , this->io_type_ ,  App_IoBridge::instance( ) );

        //添加点对点映射
        if ( true == App_IoBridge::instance()->regedit_session_id( remote_ip , this->io_type_, this->connect_id_) )
        {
            io_state_ = EM_SESSION_STATE::SESSION_IO_BRIDGE;
        }

        //查看这个链接是否有桥接信息
        this->io_bradge_connect_id_ = App_IoBridge::instance()->get_to_session_id(this->connect_id_, remote_ip );
        ...
        //添加映射关系
        App_WorkThreadLogic::instance()->add_thread_session( this->connect_id_ , shared_from_this() , local_ip , remote_ip );
